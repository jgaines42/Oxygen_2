      ...
      !   nAtomPer: number of atoms per molecule
      !   velold: velocity at last time step
      !   vel1: (natoms,3) velocities of all atoms at this time step
      !   mass: Mass of 1 atom
      !
      ! Output:
      ...
      !   nAPer: number of atoms per molecule
      !   pos: position at this time step
      !   pos_old: positions at last time step
      !   vel: velocity at 0.5 time step
      !   mass: mass of one atom
      !   tbl_SQ: target bond length squared
      !   dt: time step
      !
      ! Output:
      !   mass_term: mass term used (dummy value)
      !
      ! Modifies pos and velocity with new values after changing bl
      !-------------------------------------------------------------
     ...
     :mass,tbl_SQ,dt) result(mass_term)
        ...
        INTEGER I,JA,JB,K, dev_count
        REAL*8 thisLength_SQ              ! current bond length squared
        REAL*8 dot_prod                   ! Dot product of old bond vector and new bond vector
        REAL*8 g                          ! used to calculate correction
        REAL*8 bond_vector(3), old_bond_vector(3) ! vector of the bond
        REAL*8 mass_term                  ! mass term (based on all atoms)
        LOGICAL flag                      ! Flag if correction was enough

        mass_term=((1.0/mass)+(1.0/mass)) ! sum of (1/mass) 

        ! Loop over all molecules
        DO I=1,nMol
          dev_count=0 ! Keep track of how many times we've looped
          flag=.TRUE.
          DO WHILE(flag) ! shake
            flag=.FALSE.
            ...
            DO JA=1,nAPer-1
               DO JB=JA+1,nAPer
                  ...
                  ! Get current bond length and vector
                  thisLength_SQ=0.0
                  dot_prod=0.0
      ...
      ! Variables for VCC
      INTEGER CVV_size
      PARAMETER (CVV_size=1000)
      REAL*8 vel_store(CVV_size,nMol,3)
     ...
     :(pos(I,2,K),K=1,3),(vel(I,2,K),K=1,3)
      ...
      DO time_loop=1,nstep
        ...
        DO I=1,nMol
            DO K=1,3
              vel_store(CVV_I1,I,K)=0
            END DO
        END DO

        ! Store center of mass velocities
        DO I=1,nMol
          DO J=1,nAtomPer
            DO K=1,3
              vel_store(CVV_I1,I,K)=vel_store(CVV_I1,I,K)
     :+vel(I,J,K)*massO/massO2
            END DO
          end DO
        END DO
     ...
     :+vel_store(CVV_I1,I,K)*vel_store(CVV_I2,I,K)
        ...
        DO I=1,nMol
          DO J=1,nAtomPer
            DO K=1,3
               p(MSD_I1,I,K)=p(MSD_I1,I,K) + 
     :pos(I,J,K)*MassO/MassO2
            END DO
          END DO
         END DO
...
C              WRITE(91,*)(nMol*nAtomPer)

C              DO I=1,nMol
C                 WRITE(91,31)I,resname,atomname1,I,
C      :(pos(I,1,K)*1.0E9,K=1,3),
C      :(vel(I,1,K)*1.0E-4,K=1,3)
C                  WRITE(91,31)I,resname,atomname2,I,
C      :(pos(I,2,K)*1.0E9,K=1,3),
C      :(vel(I,2,K)*1.0E-4,K=1,3)
